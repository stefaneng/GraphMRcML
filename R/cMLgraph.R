Graph_Screen <- function(b_mat, se_mat, n_vec, IV_list = NULL, R_list, rho_mat, c_vec = rep(1, length(n_vec)), sig.cutoff = 5e-08) {
  ...
  for (i in 1:(n_trait - 1)) {
    for (j in (i + 1):n_trait) {
      ...
      if (!is.null(IV_list)) {
        SNP_used <- IV_list[[k]]
        # Filter SNPs by IV_list
        ind_X_new <- ind_X_new[is.element(ind_X_new_snp, SNP_used)]
        ind_Y_new <- ind_Y_new[is.element(ind_Y_new_snp, SNP_used)]
      }

      IJ_snp_list[[k]]$ind_i_new <- ind_X_new
      IJ_snp_list[[k]]$ind_j_new <- ind_Y_new
      k <- k + 1
    }
  }
#  browser()
  ...
  for (i in 1:m_block) {
    R <- R_list[[i]]$R
    # TODO: Maybe should just throw error here on rownames missing?
    if (!is.null(rownames(b_mat)) && !is.null(rownames(R))) {
    ...
    # TODO: Need to ensure R has rownames...
    if (is.null(rownames(R))) {
      R_rownames <- seq_len(nrow(R))
    }
    DP_mat_list[[i]]$snp <- R_rownames
  }
  ### End of screening 2 ###
...
Graph_Estimate <- function(b_mat, se_mat, n_vec, rho_mat, IJ_snp_list = NULL, t, random_start = 10) {
  ...
  for (i in 1:(n_trait - 1)) {
    for (j in (i + 1):n_trait) {
      ...
      JtoI_cML_O_res <- mr_cML_O(
        b_exp = b_mat[ind_j_new, j],
...
Graph_Perturb <- function(b_mat, se_mat, n_vec, rho_mat, IV_list = NULL, R_list, c_vec = rep(1, length(n_vec)),
  ...
  screen_res <- Graph_Screen(
    b_mat = b_mat, se_mat = se_mat,
  ...
  obs_graph_list <- lapply(out_list, function(x) {
    x$obs_graph
  })
  obs_graph_se_list <- lapply(out_list, function(x) {
    x$obs_graph_se
  })
  obs_graph_pval_list <- lapply(out_list, function(x) {
    x$obs_graph_pval
  })
  dir_graph_list <- lapply(out_list, function(x) {
    x$dir_graph
  })
